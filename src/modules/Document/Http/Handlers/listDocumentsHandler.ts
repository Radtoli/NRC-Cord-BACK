import { FastifyRequest, FastifyReply } from "fastify";
import { ObjectId } from "mongodb";
import { AuthorizationHeadersType } from "../../../User/Http/Schemas/Header/authorizationHeadersSchema";
import { DocumentIdParamType } from "../Schemas/params/documentIdParamSchema";
import { container } from "../../../../shared/infra/containers";
import { ListDocumentsService } from "../../Services/ListDocumentsService";
import { DocumentNotFoundError } from "../../Errors/DocumentErrors";

export async function listDocumentsHandler(
  request: FastifyRequest<{
    Headers: AuthorizationHeadersType;
  }>,
  reply: FastifyReply
) {
  try {
    const currentUser = request.user;

    if (!currentUser) {
      return reply.status(401).send({
        success: false,
        error: 'Unauthorized',
        message: 'User not authenticated'
      });
    }

    const listDocumentsService = container.resolve<ListDocumentsService>('listDocumentsService');

    const documents = await listDocumentsService.execute();

    return reply.status(200).send({
      success: true,
      data: documents,
      message: 'Documents retrieved successfully'
    });

  } catch (error) {
    console.error('ListDocumentsHandler Error:', error);
    return reply.status(500).send({
      success: false,
      error: 'Internal Server Error',
      message: 'Failed to retrieve documents'
    });
  }
}

export async function getDocumentByIdHandler(
  request: FastifyRequest<{
    Headers: AuthorizationHeadersType;
    Params: DocumentIdParamType;
  }>,
  reply: FastifyReply
) {
  try {
    const currentUser = request.user;

    if (!currentUser) {
      return reply.status(401).send({
        success: false,
        error: 'Unauthorized',
        message: 'User not authenticated'
      });
    }

    const listDocumentsService = container.resolve<ListDocumentsService>('listDocumentsService');

    const documentId = new ObjectId(request.params.id);
    const document = await listDocumentsService.findById(documentId);

    return reply.status(200).send({
      success: true,
      data: document,
      message: 'Document retrieved successfully'
    });

  } catch (error) {
    if (error instanceof DocumentNotFoundError) {
      return reply.status(404).send({
        success: false,
        error: 'Not Found',
        message: error.message
      });
    }

    console.error('GetDocumentByIdHandler Error:', error);
    return reply.status(500).send({
      success: false,
      error: 'Internal Server Error',
      message: 'Failed to retrieve document'
    });
  }
}